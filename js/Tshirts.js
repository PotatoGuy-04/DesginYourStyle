...
const changeImg = (imageIndex) => {
  ...
  switch (imageIndex) {
    ...
    case 7:
      ...
      break;
    case 8:
      document.querySelector('.product-select-con').value = 'catDrawingWithText';
      break;
  }
  const selectedItem = document.querySelector('.product-select-con').value;
  if (imageIndex === 6) {
...
var defaultTextBox = '"Type the name"';
const textbox = document.querySelector('.product-textbox-name');
const deleteInside = () => {
  if (textbox.value === defaultTextBox) {
    textbox.value = '';
  }
} 
...
textbox.addEventListener('focusout', function() {
  if (textbox.value === '') {
    textbox.value = defaultTextBox;
    textbox.style.color = 'gray';
  }
  else if (textbox.value !== defaultTextBox) {
